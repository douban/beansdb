...
func NewHTree(depth,pos int) *HashTree {
    t := C.ht_new((_Ctype_int)(depth), (_Ctype_int)(pos))
...
func (t *HashTree) Add(key string, item *Item) {
    ...
    C.ht_add(t.tree, p,
        (_Ctypedef_uint32_t)(pos),
        (_Ctypedef_uint16_t)(item.Hash),
        (_Ctypedef_int32_t)(item.Version))
}
...
func (t *HashTree) Hash() uint16 {
    c_key := C.CString("@")
    defer C.free(unsafe.Pointer(c_key))
    return uint16(C.ht_get_hash(t.tree, c_key, (*_Ctype_int)(nil)))
}
